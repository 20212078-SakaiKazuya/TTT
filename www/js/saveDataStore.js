...
function savePin(la, lo, im){
    if (arguments.length == 2) {
        ...
        console.log('comeback: savePinCreate');
    }
}
...
async function savePinCreate(latitude, longitude, image) {
    ...
    await pin.set("pinID", pinId)
        ...
        .then(function (pin) {
            ...
            var newPin = L.marker([map.latitude, map.longitude]).addTo(mymap);
            newPin.bindPopup("<table style='width:200px'><tr><td><div>" + pinName + '</div></td><td><img src="images/updateNameBt.png" width="20" onclick="pinRename(' + pinId + ');"></td><td><img src="images/star.png" width="20" onclick="confirmBookmarkFlg(' + pinId + ');"></td></tr></table><br><table><tr><td>アルバム</td><td >' + 1 + '</td><td>枚</td><td><img src="images/picbt.png" width="50" style="position:relative;top:2px;" onclick="pictureSelect(' + pinId + ');"></td></tr></table><div onclick="transition(' + "'album.html', " + pinId + ')"><img src="data:image/jpeg;base64,' + image + '" height="100"><span> </span><img src="images/piccount.png" width="20"></div><img src="images/delPinBt.png" width="20" style="margin-left:25%" onclick="confirmPinDelete(' + pinId + ')">');
            markers[pinId] = newPin;
        })
        .catch(function (err) {
            console.log('エラー内容: ' + err);
